...
import eyihcn.shares.utlis.CommonDaoHelper;
import eyihcn.shares.utlis.Json;
import eyihcn.shares.utlis.MyBeanUtil;
...
public class BaseMongoDao<T extends eyihcn.entity.BaseEntity<PK>, PK extends Serializable>
		implements CommonDaoInter<T, PK> {
	...
	public Boolean delete(T entity) {
		try {
			...
			delete(Criteria.where("_id").is(entity.getId()));
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean delete(Map<String, Object> requestArgs) {
		return delete(getRequestRestriction((Map<String, Object>) requestArgs.get(QUERY_KEY)));
	}
	...
	public boolean update(Map<String, Object> updateParam) {
		...
		try {
			...
			if (null != updates) {
				...
				updateParam.remove("_id");
				updateParam.remove("id");
				updateParam.remove("_class");
			...
			findAndModify(Criteria.where(_ID).is(id), update);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private Criteria _parseRequestRestrictionOr(Map<String, Object> query) {
		...
		if (!criterias.isEmpty()) {
			allOrCriteria.orOperator(criterias.toArray(new Criteria[criterias.size()]));
		}
	...
	public Criteria getRequestRestriction(Map<String, Object> query) {
		...
		if (!criterias.isEmpty()) {
			allCriteria.andOperator(criterias.toArray(new Criteria[criterias.size()]));
		}
		return allCriteria;
	}
	...
	public Query getQueryFromQueryParam(Map<String, Object> queryParam) {
		// 无参 ，默认是{}
		Criteria criteria = getRequestRestriction((Map) queryParam.get(QUERY_KEY));
		// 无参 ，默认是id
	...
	public Long findCollectionCount(Map<String, Object> requestArgs) {
		Criteria criteria = getRequestRestriction((HashMap<String, Object>) requestArgs.get(QUERY_KEY));
		return Long.valueOf(count(criteria));
	}

	/** 根据ID批量更新 */
	public boolean batchUpdateByIds(List<PK> ids, Map<String, Object> updates) {
		try {
			...
			WriteResult writeResult = updateMulti(Criteria.where(_ID).in(ids), update);
			if (null == writeResult) {
				return false;
			}
			return writeResult.getN() > 0 ;
	...
	public BasicDBList group(Map<String, Object> requestArgs) throws Exception {
		...
		String groupKey = groupConditions.get("key");
		String groupInitialDocument = groupConditions.get("initialDocument");
		String groupReduceFunction = groupConditions.get("reduceFunction");
